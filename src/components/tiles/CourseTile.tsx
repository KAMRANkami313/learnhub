...
export default function CourseTile({ course, index }: CourseTileProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border relative overflow-hidden grain-overlay group cursor-pointer"
      style={{
    ...
    >
      ...
      <div className="relative z-10 p-5">
        ...
        {/* Footer: Lessons count + Progress */}
        <div className="flex items-center justify-between">
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            {course.completed_lessons} / {course.total_lessons} lessons
          </p>
          <p className="text-xs font-semibold" style={{ color: course.color }}>
            {course.progress}%
          </p>
        </div>
      </div>
